function [ objVal, v, lambda ] = roSolveModelWithConcentrations( epsilon, eigenLb, model, groups, rho, quiet )
%ROSOLVEMODELWITHCONCENTRATIONS Solves the supplied ZavRO problem using the 
%CVX package. In this scenario the stoichiometry is augmented by a
%concentration term (dc / dt) which prevents the network from creating 
%substrate from the void.
%
% Reference: 
...
% Augment S with concentrations
model.S = [model.S, -eye(nMets)];
model.cM = [model.cM, zeros(size(model.cM, 1), nMets)];
model.lb = [model.lb; zeros(nMets, 1)];
model.ub = [model.ub; inf(nMets, 1)];
model.c = [model.c; zeros(nMets, 1)];
...
%    cvx_solver sdpt3;
    ...
    subject to
        ...
        v(~isinf(model.ub)) <= model.ub(~isinf(model.ub));
        v >= model.lb;
