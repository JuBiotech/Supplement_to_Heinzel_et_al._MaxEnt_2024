function [ res ] = rangeGroupsRelative( model, group, delta, steps )
%RANGEGROUPSRELATIVE Samples a given parameter range and records the
%results.
...
%	- solutions: Matrix in which each column is a solution to the FBA
	...
	origSol = glpkInterface(-1, model.c, [], [], S, model.b, model.lb, model.ub);
	c = 2*delta / steps;
	...
	for i = 1:steps+1
		...
		[sol(:, i), val(i)] = glpkInterface(-1, model.c, [], [], S_neu, model.b, model.lb, model.ub);
